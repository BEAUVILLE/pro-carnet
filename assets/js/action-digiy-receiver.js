...
(function(){
  "use strict";
  const VERSION="action-digiy-receiver-v5-global-module-clean-url-safe-html-20260530";
  ...
  function cleanUrl(){
    ...
    try{
      ...
      if(url.hash){
        ...
        if(params.has("digiyAction")||params.has("action")){
          params.delete("digiyAction");
  ...
  function copyText(t){
    t=String(t||"");
    if(!t)return;
    if(navigator.clipboard&&navigator.clipboard.writeText)navigator.clipboard.writeText(t).catch(()=>prompt("Copie :",t));
    else prompt("Copie :",t);
  }
  ...
  function fillFields(action){
    ...
    notes.forEach(s=>put(s,note));
    amounts.forEach(s=>put(s,amount));
    channels.forEach(s=>put(s,channel));
  }
  ...
  function mountPanel(action){
    ...
    panel.querySelector("[data-save]").addEventListener("click",()=>{
      saveAction(action,"received_draft");
    ...
    panel.querySelector("[data-fill]").addEventListener("click",()=>{
      fillFields(action);
      saveAction(action,"received_draft");
      alert("Brouillon pré-rempli. Vérifie puis valide.");
    });
    panel.querySelector("[data-validate]").addEventListener("click",()=>{
      const validated=validateDraft(action);
      const title=panel.querySelector(".digiyRecvTitle");
      if(title)title.textContent="Brouillon validé.";
      const box=panel.querySelector(".digiyRecvNote");
      if(box){
        const safeValidatedNote=escapeHtml(cleanCommand(validated.commandText||validated.rawText||""));
        box.innerHTML="<strong>Brouillon validé :</strong><br>"+safeValidatedNote+"<br><br>Le pro a confirmé ce brouillon ACTION DIGIY. Vérifie la page métier puis enregistre si nécessaire.";
      }
      alert("Brouillon validé dans "+moduleLabel(MODULE)+". Vérifie la page métier puis enregistre si nécessaire.");
    });

    const target=document.querySelector("main")||document.querySelector(".page")||document.querySelector(".shell")||document.body;
    if(target===document.body)document.body.insertBefore(panel,document.body.firstChild);
