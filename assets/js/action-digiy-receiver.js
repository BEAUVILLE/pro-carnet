...
(function(){
  ...
  function mountPanel(action){
    ...
    const target=document.querySelector("main")||document.querySelector(".page")||document.querySelector(".shell")||document.body; if(target===document.body)document.body.insertBefore(panel,document.body.firstChild); else target.insertBefore(panel,target.firstChild);
  }

  function boot(){ const incoming=readActionFromUrl(); if(!incoming)return; const saved=saveAction(incoming,"received_draft"); cleanUrl(); mountPanel(saved); window.dispatchEvent(new CustomEvent("digiy:action-received",{detail:saved})); }
